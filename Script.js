...
async function handleCollageFiles(files) {
    ...
    generateCollagePreview(0.5);
}

imgSizeInput.addEventListener('input', (e) => { imgSizeNum.value = e.target.value; generateCollagePreview(0.5); });
imgSizeNum.addEventListener('input', (e) => { imgSizeInput.value = parseInt(e.target.value) || 50; generateCollagePreview(0.5); });
columnsInput.addEventListener('input', (e) => { columnsVal.textContent = e.target.value; generateCollagePreview(0.5); });
imgGapInput.addEventListener('input', (e) => { imgGapVal.textContent = `${e.target.value}px`; generateCollagePreview(0.5); });
strokeWidthInput.addEventListener('input', (e) => { strokeWidthVal.textContent = `${e.target.value}px`; generateCollagePreview(0.5); });
strokeColorInput.addEventListener('input', () => generateCollagePreview(0.5));
...
// ==========================================
// 3. منطق استوديو السبرايت (Sprite Studio)
// ==========================================
...
// التفاعل مع الكانفاس لتبديل الأماكن وحذف الفريمات
        ...
        tempVideo.onloadedmetadata = async () => {
            ...
            for (let i = 0; i < totalFrames; i++) {
                ...
                await new Promise((r) => {
                    ...
                    setTimeout(handler, 400); // حماية من التجمد
                });
